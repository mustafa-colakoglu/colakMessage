<?php
	class CLKMessage{
		...
		*/
		...
		public function connectDb($server,$user,$pass,$db=false){
			$this->data["mysql"]=array($server,$user,$pass,$db);
		...
		public function checkUser($userName){
			if(mysql_num_rows(mysql_query("SELECT * FROM kullanicilar WHERE kullaniciAdi='$userName'"))<1){
				return false;
			}
			return true;
		}
		public function addUser($userName,$password,$secretAnswer){
			if(!$this->checkUser($userName)){
				mysql_query("INSERT INTO kullanicilar VALUES('','$userName','$password','$secretAnswer')");
				return true;
			}
			else{
				return false;
		...
		public function changePassword($userId,$newPassword){
			if(mysql_num_rows(mysql_query("SELECT * FROM kullanicilar WHERE id='$userId'"))<1){
				return false;
			}
			else{
				mysql_query("UPDATE kullanicilar SET password='$newPassword'");
				return true;
			}
		}
		public function changeSecretAnswer($userId,$newAnswer){
			if(mysql_num_rows(mysql_query("SELECT * FROM kullanicilar WHERE id='$userId'"))<1){
				return false;
			}
			else{
				mysql_query("UPDATE kullanicilar SET password='$newAnswer'");
				return true;
			}
		}
		public function userLogin($userName,$password){
			$loginControl=@$_SESSION["CLKId"];
			if(!$loginControl){
				$login=mysql_query("SELECT * FROM kullanicilar WHERE kullaniciAdi='$userName' and sifre='$password' LIMIT 1");
				if(mysql_num_rows($login)<1){
					$this->data["userLoginFail"]=2;
					return false;
				}
				else{
					$login=mysql_fetch_array($login);
					$_SESSION["CLKId"]=$login["id"];
					$_SESSION["CLKUserName"]=$login["kullaniciAdi"];
					$this->data["userLoginFail"]=1;
					return true;
				}
			}
			else{
				$this->data["userLoginFail"]=3;
				return false;
			}
		}
		public function userLogout(){
			if(!@$_SESSION["CLKId"] or !@$_SESSION["CLKUserName"]){
				return false;
			}
			else{
				$_SESSION["CLKId"]=false;
				$_SESSION["CLKUserName"]=false;
				return true;
			}
		}
		public function isLogin(){
			if(@$_SESSION["CLKId"] and @$_SESSION["CLKUserName"]){
				return true;
			}
			else{
				$this->userLogout();
